...
public class RpcServerFactoryPBImpl implements RpcServerFactory {
  ...
  private String getPbServiceImplClassName(Class<?> clazz) {
    // 根据接口协议的名称，查找实现类的名称
    // clazz(ApplicationClientProtocol)---包名(org.apache.hadoop.yarn.api)
    String srcPackagePart = getPackageName(clazz);
    String srcClassName = getClassName(clazz);
    // 拼接包名org.apache.hadoop.yarn.api.impl.pb.service
    String destPackagePart = srcPackagePart + "." + PB_IMPL_PACKAGE_SUFFIX;
    // 拼接 实现类名 ApplicationClientProtocolPBServiceImpl
    String destClassPart = srcClassName + PB_IMPL_CLASS_SUFFIX;
    // 拼接全限定类名 org.apache.hadoop.yarn.api.impl.pb.service.ApplicationClientProtocolPBServiceImpl
